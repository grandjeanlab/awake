...
def get_masker(tmp_listdir, x, label_img, roiL, roiR):
  try:
    x = x.split("_")
    sub = x[0]
    ses = x[1]
    ...
    if scan_file.is_empty():
      ...
      roi_values = masker.fit_transform(scan_file[0]).astype(np.float32)
      keyL = [key for key, val in masker.region_ids_.items() if val == roiL]
      keyR = [key for key, val in masker.region_ids_.items() if val == roiR]
      return pl.Series(roi_values)[0][[keyL[0],keyR[0]]]
  except:
    return pl.Series([None, None], dtype=pl.Float32)
...
gsr_cov_img_dir = "data_diagnosis_datasink/GS_cov_nii"
...
if rodent == "rat":
  roiL = 521  # left barrel field cortex
  roiR = 522  # right barrel field cortex
if rodent == "mouse":
  roiL = 281
  roiR = 117
...
tmp_listdir = tmp_listdir + tmp_listdir_alt 
tmp_listdir = pl.Series(tmp_listdir)
tmp = df.with_columns(pl.col("scan").map_elements(lambda x: get_masker(tmp_listdir, x, label_img, roiL, roiR)).alias('roi')).select('scan_dir','roi')
...
for analysis in analysis_list:
  ...
  p = join(data_dir, analysis, gsr_cov_img_dir)
  tmp_listdir = glob.glob(join(p,"*/*/*.nii.gz"), recursive=True)
  tmp_listdir_alt = glob.glob(join(p,"*/*.nii.gz"), recursive=True)
  tmp_listdir = tmp_listdir + tmp_listdir_alt 
  tmp_listdir = pl.Series(tmp_listdir)
  ...
  df = df.join(tmp.with_columns([
    ...
    ]).select("scan_dir", "s1.gsrcov.l."+analysis, "s1.gsrcov.r."+analysis), on="scan_dir")
