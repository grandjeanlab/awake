...
#now we run the analysis per denoising style, we extract the number of dropped frames, the s1-s1, s1-aca, and s1-thal correlations. finally we estimate connectivity specificity
  ...
  print(df_summary[["rodent.ds"]+["s1.cat."+analysis+".Specific" for analysis in analysis_list]].sort(by="rodent.ds"))

  print("it seems that less scrubbing lead to better outcomes")
  print("for gsr processing, "+str((df_summary["s1.cat.gsr1.Specific"] < df_summary["s1.cat.gsr3.Specific"]).sum())+"/"+str(df_summary["s1.cat.gsr1.Specific"].count())+" dataset performed better with less scrubbing")
  print("for wmcsf processing, "+str((df_summary["s1.cat.wmcsf1.Specific"] < df_summary["s1.cat.wmcsf3.Specific"]).sum())+"/"+str(df_summary["s1.cat.wmcsf1.Specific"].count())+" dataset performed better with less scrubbing")
  print("for comparing gsr to wmcsf processing, "+str((df_summary["s1.cat.gsr3.Specific"] < df_summary["s1.cat.wmcsf3.Specific"]).sum())+"/"+str(df_summary["s1.cat.wmcsf3.Specific"].count())+" dataset performed better with wmcsf compared to gsr")
  print("for comparing gsr to wmcsf processing, "+str((df_summary["s1.cat.gsr3.Specific"] > df_summary["s1.cat.wmcsf3.Specific"]).sum())+"/"+str(df_summary["s1.cat.wmcsf3.Specific"].count())+" dataset performed worst with wmcsf compared to gsr")
...
### adding the dice analysis ####
  ...
  #plot the relation between categories and dice score for s1 and aca
  plt.figure(figsize=(10,5))
  ax = sns.jointplot(x=df["dice_s1"], y=df["dice_aca"], hue=df[f"s1.cat.{analysis}"])
  ax.figure.savefig("../assets/plot/"+rodent+"dice_s1_vs_dice_aca.svg")
  ...
  ax.figure.savefig("../assets/plot/"+rodent+"dice_s1_per_dataset.svg")

  plt.figure(figsize=(10,5))
  ax = makeswarmplot('Dice ICA ACA per dataset', df["rodent.ds"], df["dice_aca"], hue=df['head-plate'])
  ax.figure.savefig("../assets/plot/"+rodent+"dice_aca_per_dataset.svg")
