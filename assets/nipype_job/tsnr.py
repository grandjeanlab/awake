...
def keep_filename(out_paths):
  return out_paths.split('/')[6] + out_paths.split('/')[7] + '.tsv'
...
df_node.inputs.root_paths = join(data_dir, tsnr_img_dir)
...
ds_node.inputs.base_directory = join(ds_dir, 'tsnr')
ds_node.inputs.parameterization = False

#define the workflow
wf = Workflow(name='workflow_tsnr')
wf.base_dir = wf_dir
